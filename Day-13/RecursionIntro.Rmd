...
```{r}
integrateRecursive<- function(f, a, b, bLIM = (b-a)*1e-4){
  

  bigBins<- simpleRiemann(f,a=a,b=b, n=5)  
  smallBins<- simpleRiemann(f,a=a,b=b,n=10)
  if ((abs(bigBins - smallBins)< 1e-04)){ #| ( (b-a)/10 < bLIM)){
return(smallBins)    
    }else{
      if( (b-a)/10 < bLIM){
    ...
    mid<-(a+b)/2
    total<- integrateRecursive(f,a=a,b=mid,bLIM = bLIM)+integrateRecursive(f,a=mid,b=b,bLIM = bLIM)
    return(total)
    }}
  }
```
...
integrateRecursive(function(x){x^6},1,10,)
```
Testing with $x^{5}$
Integral of  $x^{5}$ is $\frac{x^{6}}{6}$
```{r}
integral2<- (10)^6/6  - 1^6/6
integrateRecursive(function(x){x^5},1,10,) 
integrateRecursive(function(x){x^5},1,10,) == integral2
```
...
```{r}
integral3 <- 10  - 1
integrateRecursive(function(x){1},1,10,)
integrateRecursive(function(x){1},1,10,)==integral3
```
